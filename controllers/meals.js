...
async function updateMeal(meal) {
  try {
    ...
    return {meal: makeItem(meal,['id','name','price','category',...keysToUpdate])}
  }catch(err) {
    deleteFiles(meal.picture);
    throw await getError(err);
  }
 ...
 * @param {String} id
 */
async function getMeals(id=null,attributes=['id','name','category','price']) {
    let meals =[];
    if (!id) {
    ...
    return {count:meals.length,data:meals.map(meal => makeItem(meal,attributes))};
}
